...
const log = createDefaultLogger({
  ...
  bindings: { project: 'treasury-rebalance', script: 'stake' },
});

if (treasury.DEMO_MODE) {
  log.warn(
    {},
    '[treasury-rebalance] DEMO_MODE=true — staking is a no-op in demo mode. Set DEMO_MODE=false to stake on-chain.',
  );
  process.exit(0);
}
...
log.info(
  ...
  '[treasury-rebalance] submitting stake',
);
...
function parseStakeArg(arg: string | undefined): bigint {
  ...
  try {
    ...
    console.error(
      `[treasury-rebalance] invalid stake amount "${arg}". Pass a positive integer wei value.`,
    );
